...
| Branch | Purpose |
|--------|---------|
| `main` | Tracks upstream [margelo/react-native-nitro-sqlite](https://github.com/margelo/react-native-nitro-sqlite) |
| `paperpile/custom-extension` | Paperpile's custom code (development branch) |
| `paperpile/release` | Auto-generated branch for npm installation |

## Installation

```bash
# In your package.json
"react-native-nitro-sqlite": "github:Paperpile/react-native-nitro-sqlite#paperpile/release"
```
...
The `paperpile/release` branch is automatically generated by a GitHub Action whenever changes are pushed to `paperpile/custom-extension`. This branch contains the `package/` directory contents at root level, making it directly installable via npm/yarn.

```
paperpile/custom-extension          paperpile/release
├── example/                        ├── package.json  ← at root
├── package/           ──────►      ├── src/
│   ├── package.json    (auto)      ├── lib/
│   └── src/                        ├── ios/
└── README.md                       └── android/
```
...
git checkout paperpile/custom-extension
git merge main
git push origin paperpile/custom-extension
# → GitHub Action auto-updates paperpile/release
```

## Manual Workflow Trigger

```bash
gh workflow run sync-release-branch.yml --repo Paperpile/react-native-nitro-sqlite --ref paperpile/custom-extension
```
